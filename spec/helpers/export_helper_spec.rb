...
RSpec.describe ExportHelper, type: :helper do
  describe 'creates language export data correctly' do
    it 'create language data correctly with for language with parent' do
      ...
      language_child_data =
        helper.create_language_export_data(project, export_config, language_child, [], skip_timestamp: true)
      language_parent_data =
        helper.create_language_export_data(project, export_config, language_parent, [], skip_timestamp: true)
      language_no_connection_data =
        helper.create_language_export_data(project, export_config, language_no_connection, [], skip_timestamp: true)
      ...
      expect(language_child_data).to match_snapshot('export_helper_spec_language_with_parent_child_data')
      expect(language_parent_data).to match_snapshot('export_helper_spec_language_with_parent_parent_data')
      expect(language_no_connection_data).to match_snapshot(
        'export_helper_spec_language_with_parent_no_connection_data'
      )
    end
  end
end
