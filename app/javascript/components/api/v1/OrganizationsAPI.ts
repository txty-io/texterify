...
const OrganizationsAPI = {
  ...
  deleteImage: async (options: { organizationId: string }): Promise<any> => {
    return API.deleteRequest(`organizations/${options.organizationId}/image`, true).then(APIUtils.handleErrors).catch(APIUtils.handleErrors);
  },

  uploadImage: async (options: { organizationId: string; formData: FormData }): Promise<any> => {
    return API.postRequest(`organizations/${options.organizationId}/image`, true, options.formData, null, true).then(APIUtils.handleErrors).catch(APIUtils.handleErrors);
  }
};
