...
class TopBarUnwrapped extends React.Component<IProps, IState> {
    routes: any[] = [
        ...
        },
        {
            ...
            name: (
                <Button type="primary" icon={<RocketOutlined />} style={{ boxShadow: "0 0 10px 0 rgba(0, 0, 0, .15)" }}>
                    Get started
                </Button>
            ),
            notAuthenticatedOnly: true
            ...
            >
                ...
                >
                    <div style={{ flexGrow: 1 }}>
                        ...
                        <p style={{ fontSize: 13, marginBottom: 0 }}>The easy way to localize.</p>
                    </div>
    ...
    renderRoutes = (): JSX.Element[] => {
        return this.routes.map((route: any, index: number) => {
            ...
            if (route.route) {
                ...
                return (
                    <span key={index} className="nav-link" onClick={route.onClick} role="button">
                        {route.name}
                    </span>
                );
            }
