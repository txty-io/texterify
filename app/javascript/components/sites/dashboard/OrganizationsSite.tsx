...
class OrganizationsSiteUnwrapped extends React.Component<IProps, IState> {
  ...
  getRows = (): any[] => {
    ...
    return this.state.organizationsResponse.data && this.state.organizationsResponse.data.map((organization: any) => {
      return {
        key: organization.id,
      ...
      <>
        <Layout style={{ padding: "0 24px 24px", maxWidth: 800, margin: "0 auto", width: "100%" }}>
          <Layout.Content style={{ margin: "24px 16px 0", minHeight: 360 }}>
            ...
            <List
              size="small"
              locale={{ emptyText: <Empty description="No organizations found" image={Empty.PRESENTED_IMAGE_SIMPLE} /> }}
                ...
                <List.Item key={item.title}>
                  <List.Item.Meta
                    title={
                      <ListContent
                        onClick={(): void => {
                          this.openOrganization(_.find(this.state.organizationsResponse.data, { id: item.key }));
                        }}
                        role="button"
                      >
                        <OrganizationAvatar
                          organization={_.find(this.state.organizationsResponse.data, { id: item.key })}
        ...
        <NewOrganizationFormModal
          ...
          newOrganizationFormProps={{
            onCreated: (organizationId: string) => {
              this.props.history.push(Routes.DASHBOARD.ORGANIZATION.replace(":organizationId", organizationId));
            },
            onError: (errors) => {
